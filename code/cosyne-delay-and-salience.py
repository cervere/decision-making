...
if getWeights:
    ...
    ptype = [("[1, 0]", [("salience", float, (np.size(saliences), run_sessions)),("delay", float, (np.size(delays), run_sessions))]),
             ("[2, 0]", [("salience", float, (np.size(saliences), run_sessions)),("delay", float, (np.size(delays), run_sessions))]),
             ("[3, 0]", [("salience", float, (np.size(saliences), run_sessions)),("delay", float, (np.size(delays), run_sessions))]),
             ("[2, 1]", [("salience", float, (np.size(saliences), run_sessions)),("delay", float, (np.size(delays), run_sessions))]),
             ("[3, 1]", [("salience", float, (np.size(saliences), run_sessions)),("delay", float, (np.size(delays), run_sessions))]),
             ("[3, 2]", [("salience", float, (np.size(saliences), run_sessions)),("delay", float, (np.size(delays), run_sessions))])]
    num_trials = 20
    weights_ses = np.load("cog-weights.npy")
    performance_sal_del = np.zeros(1, dtype=ptype)
    for stim in possible_cues: 
        #stim = [2,0] #possible_cues[0]
        mean_perf_salience = np.zeros((np.size(saliences), run_sessions))
        for d in range(np.size(saliences)):
            salience = saliences[d]
            print "Simulation for %s with salience difference - %f" % (str(stim), salience)
            for i in range(run_sessions):
                P, R = [], []
                W[...] = weights_ses[i] 
                run_session(stim, delay, salience)
                mean_perf_salience[d,i] = np.array(P).mean()
        performance_sal_del[str(stim)]["salience"] = mean_perf_salience    
        
    #np.save("mean_perf_salience.npy", mean_perf_salience)
    print mean_perf_salience
    # Introducing more rewarding stimulus with a delay
    salience = 0
    for stim in possible_cues:
        mean_perf_delay = np.zeros((np.size(delays), run_sessions))
        for d in range(np.size(delays)):
            delay = delays[d]
            print "Simulation for %s with delay - %d (ms)" % (str(stim), delay)
            for i in range(run_sessions):
                P, R = [], []
                W[...] = weights_ses[i] 
                run_session(stim, delay, salience)
                mean_perf_delay[d,i] = np.array(P).mean()
