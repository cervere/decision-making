...
connections = [
    OneToOne( CTX.cog.V, STR.cog.Isyn, weights_ses[0],   gain=+1.0 ),
...
def start_trial(salience=0, first=True):
    ...
    CTX.cog.Iext[c1]  = (1+salience) * (v + np.random.normal(0,v*noise))
...
for j in range(1):
    ...
    for i in xrange(0,500):
        iterate(dt)
    start_trial(salience)
    for i in xrange(500,2500):
        if i == (500+delay):
            start_trial(salience, first=False)
        iterate(dt)
    print np.argmax(CTX.cog.V)
    #stop_trial()
