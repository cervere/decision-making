...
connections = [
    ...
    AssToCog(STR.ass.U, GPI.cog.Isyn, gain=-2*2.0*gpiToStr*gpiToStr),
    AssToMot(STR.ass.U, GPI.mot.Isyn, gain=-2*2.0*gpiToStr*gpiToStr),
...
for j in range(num_trials):
    ...
    for i in xrange(i0,i1):
        iterate(dt)
        if not decision and CTX.mot.delta > threshold:
            decision = True
