...
AssToCog(STR.ass('Z'), GPI.cog('Isyn'), gain=-2*2.0)
AssToMot(STR.ass('Z'), GPI.mot('Isyn'), gain=-2*2.0)
OneToAll(STN.cog('Z'), GPI.cog('Isyn'), gain=+1.0)
OneToAll(STN.mot('Z'), GPI.mot('Isyn'), gain=+1.0)
...
def register(t):
    ...
    if decision_not_made and (delta > 40.0):
